...
class Validador implements Runnable {
	...
	public Validador(int s, int e, String algoritmo) throws NoSuchAlgorithmException {
		start = s;
		end = e;
	...
	public String[] concatenar(String candidato) {
		...
		rta[0] =  candidato + Prototipo.CADENA_INICIAL;
		rta[1] = Prototipo.CADENA_INICIAL + candidato;
		return rta;
	}
	...
	public void generate(StringBuilder sb, int n) {
		...
		if (n == sb.length()) {
			...
			for (int i = 0; i < Prototipo.numCeros; i++) {
				if(a1.charAt(i) != '0') {
					funciono1 = false;
				}
				if(a2.charAt(i) != '0') {
		...
		for (int i = 0; i < Prototipo.ALPHABET.length && !DONE; i++) {
			char letter = Prototipo.ALPHABET[i];
